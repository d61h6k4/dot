...
def install_brew():
    if platform.system() == 'Darwin':
        if shutil.which('brew') is None:
            eval_os_cmd('/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install.sh)"')
    else:
...
def install_fira_code():
    ...
    if platform.system() == 'Linux':
        ...
        if rcode:
            ...
            eval_os_cmd("brew tap homebrew/cask-fonts && brew cask install font-fira-code")
...
def install_zsh_plugins():
    ...
    if not zsh_folder.exists():
        zsh_folder.mkdir()
    rcode, msg = eval_os_cmd(f"git clone https://github.com/zsh-users/zsh-autosuggestions {zsh_folder}/zsh-autosuggestions")
    if rcode:
        warnings.warn(msg)

    rcode, msg = eval_os_cmd(f"git clone --recursive https://github.com/joel-porquet/zsh-dircolors-solarized {zsh_folder}/zsh-dircolors-solarized")
    if rcode:
        logging.critical(msg)
...
def install_starship():
    ...
    if platform.system() == 'Darwin':
        ...
        rcode, msg = eval_os_cmd('curl -fsSL https://starship.rs/install.sh | bash')
        if rcode:
...
def generate_alaises(dot_folder):
    ...
    with open(alias_conf_src, 'w') as src:
        ...
        ls_cmd = 'ls=exa'
        find_cmd = 'find=fd'
        src.write('\n'.join([f'alias {cmd}' for cmd in [ls_cmd, dircolors_cmd, find_cmd]]))

    if alias_conf_dst.exists():
        if alias_conf_dst.is_symlink() and alias_conf_dst.resolve() == alias_conf_src:
            logging.info(f'{alias_conf_dst} is alread symlink to the {alias_conf_src}')
        else:
